...
1. 本リポジトリに含まれる fluidsynth フォルダ、または公式サイトからダウンロードします。
2. ダウンロードした fluidsynth フォルダを、C:\tools\ ディレクトリ内に配置します。
（最終的に C:\tools\fluidsynth\bin\fluidsynth.exe というパスになるようにしてください。toolsフォルダがない場合は作成してください。）
...
1.  **ファイル読み込み**
    -   「楽譜 (MusicXML)」と「演奏データ (MIDI)」のそれぞれに、対応するファイルを選択します。
    -   [読み込み開始] ボタンをクリックします。
2.  **パート選択**
...
3.  **フレーズ選択**
    ...
    -   フレーズの終了音符をクリックすると、頂点の候補となる音符が<span style="color: gold;">黄色</span>で示唆されます。
4.  **表現プリセット選択**
    -   適用したい発想標語をドロップダウンから選択します。
5.  **楽譜へ反映**
    -   [楽譜へ反映] ボタンをクリックします。選択したフレーズにハイライトと発想標語が追加されます。
    -   この操作は即座に完了し、[一つ前に戻る (Undo)] や [やり直す (Redo)] も高速に行えます。
6.  **音源を生成**
